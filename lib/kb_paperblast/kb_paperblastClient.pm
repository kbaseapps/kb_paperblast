package kb_paperblast::kb_paperblastClient;
...
kb_paperblast::kb_paperblastClient
...
sub new
{
    ...
    my $self = {
	client => kb_paperblast::kb_paperblastClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
# Authentication: required
	...
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function paperblast_seq (received $n, expecting 1)");
    }
    {
	my($params) = @args;
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "kb_paperblast.paperblast_seq",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method paperblast_seq",
...
sub version {
    ...
    if ($result) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(
            error => "Error invoking method paperblast_seq",
            status_line => $self->{client}->status_line,
            method_name => 'paperblast_seq',
        );
    }
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for kb_paperblast::kb_paperblastClient\n";
    }
    if ($sMajor == 0) {
        warn "kb_paperblast::kb_paperblastClient version is $svr_version. API subject to change.\n";
    }
}
...
package kb_paperblast::kb_paperblastClient::RpcClient;
