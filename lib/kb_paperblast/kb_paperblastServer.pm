package kb_paperblast::kb_paperblastServer;
...
sub call_method {
    ...
    my $ctx = kb_paperblast::kb_paperblastServerContext->new($self->{loggers}->{userlog},
...
{
    # Service kb_paperblast requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for kb_paperblast but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = kb_paperblast::kb_paperblastServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service kb_paperblast.");
    }
...
package kb_paperblast::kb_paperblastServerContext;

use strict;

=head1 NAME

kb_paperblast::kb_paperblastServerContext
...
package kb_paperblast::kb_paperblastServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = kb_paperblast::kb_paperblastServer->new(
